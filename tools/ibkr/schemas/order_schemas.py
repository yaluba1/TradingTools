...
class OrderRequest(BaseModel):
    ...
    acctId: Optional[str] = Field(None, description="Account ID associated with the order")
...
class LiveOrder(BaseModel):
    ...
    cumFill: float = Field(0.0, validation_alias=AliasChoices('cumFill', 'filledQuantity'), description="Cumulative filled quantity")
    avgPrice: float = Field(0.0, description="Average fill price")
    status: str = Field(..., description="PendingSubmit, Filled, Cancelled, etc.")
    lmtPrice: Optional[float] = Field(None, validation_alias=AliasChoices('lmtPrice', 'price'), description="Limit price")
    ...
    account: str = Field(..., validation_alias=AliasChoices('account', 'acct', 'acctId'), description="Account ID")
...
class SuppressRepliesRequest(BaseModel):
    """
    Request schema for suppressing order reply messages.
    """
    messageIds: List[str] = Field(..., description="List of message/warning IDs to suppress")

class WhatIfAmount(BaseModel):
    """
    Detailed order amount estimates from the whatif preview response.
    """
    amount: Optional[str] = Field(None, description="Projected order amount (e.g. '1,551 USD (100 Shares)')")
    commission: Optional[str] = Field(None, description="Estimated commission cost (e.g. '1.00 USD')")
    total: Optional[str] = Field(None, description="Estimated total cost including commissions (e.g. '1,552.00 USD')")

class WhatIfMetricChange(BaseModel):
    """
    Changes to account metrics (equity, margin, position) projected by the whatif preview.
    """
    current: Optional[str] = Field(None, description="Current value of the metric before order placement")
    change: Optional[str] = Field(None, description="Projected change in the metric")
    after: Optional[str] = Field(None, description="Projected value of the metric after order execution")

class WhatIfResponse(BaseModel):
    """
    Schema for the response from the order preview / whatif endpoint.
    """
    amount: Optional[WhatIfAmount] = Field(None, description="Projected order amount and commission estimates")
    equity: Optional[WhatIfMetricChange] = Field(None, description="Projected changes to equity")
    initial: Optional[WhatIfMetricChange] = Field(None, description="Projected changes to initial margin requirements")
    maintenance: Optional[WhatIfMetricChange] = Field(None, description="Projected changes to maintenance margin requirements")
    position: Optional[WhatIfMetricChange] = Field(None, description="Projected changes to position size")
    warn: Optional[str] = Field(None, description="First warning message if any warnings were generated")
    error: Optional[str] = Field(None, description="Error message if the preview failed")
    warns: Optional[List[str]] = Field(None, description="All warning messages generated by the order preview")
    accruedInterest: Optional[str] = Field(None, description="Estimated accrued interest for the order")
